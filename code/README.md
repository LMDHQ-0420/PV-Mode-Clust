# 天气区制感知的可学习软门控异质多专家光伏日前功率预测（NWP 误差自适应订正 + 互补 Stacking）

> 关联设计：`../docs/idea_report.md`（idea / 实验设计）、`../docs/implementation.md`（实现指南）

## 项目主要内容

围绕 PVOD（河北 10 站，含 NWP 预报 + LMD 实测）数据，做**日前 96 点（15min × 1 天）光伏功率预测**。方法由五模块组成（A–D 单阶段端到端联合，E 集成）：

- **A. FCM 软天气区制**：仅用 NWP 侧气象特征做模糊 C 均值聚类，产出软隶属 `u(t)`。
- **B. 自适应 NWP 订正**：以 `[NWP辐照, u]` 为输入，经残差门控 `γ` **仅在大误差区订正辐照**（残差+幅度约束），作端到端辅助损失。
- **C. 结构异质专家**：**以未来段订正 NWP 序列为主序列**，局部卷积 / 膨胀 TCN / 逐步 MLP 三类对应不同天气物理，历史编码为上下文条件。
- **D. 可学习软门控**：`g=softmax(α·log u + MLP(NWP))`，FCM 软隶属作可解释先验 + 可学习残差，软混合各专家。
- **E. 互补 Stacking**：将训好的深度区制 MoE 与 RF 按**验证集拟合的凸组合权重**融合为最终预测——SOTA 最终模型。

单阶段端到端联合训练 A–D（总损失 `L=L_pred+λ·L_corr`），再做 E 的 stacking。

> 实证结论（10站×5种子，日前同口径）：deep 88.77 / RF 90.22 / **stack 90.51**（平均 +0.29 vs RF，8/10站正增益，同时压方差）。
